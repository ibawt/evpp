...
namespace ev {
  b2DebugDraw::b2DebugDraw(float ratio) : mShader(vertex_shader, fragment_shader), mColorLocation(0), mRatio(ratio),
                                          vbuff(1024*1024)
...
void b2DebugDraw::DrawPolygon(const b2Vec2* vertices, int cnt, const b2Color& color)
{
  ...
  glUniformMatrix4fv( mShader.get_uniform_loc("u_projTrans"),
                      1, GL_FALSE, glm::value_ptr(mMatrix));

  glUniform4f(mShader.get_uniform_loc("u_color"),
              color.r, color.g, color.b, 1.0f);

  glUniform1f(mShader.get_uniform_loc("ratio"),
              mRatio);

  glVertexAttribPointer(mShader.get_attrib_loc("a_position"),
